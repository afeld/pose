...
const evalModelOnDatasetButton = document.getElementById(
...
const transferModelSaveLoadInnerDiv = document.getElementById(
  "transfer-model-save-load-inner"
);
...
const savedTransferModelsSelect = document.getElementById(
  "saved-transfer-models"
);
const deleteTransferModelButton = document.getElementById(
  "delete-transfer-model"
);
...
const includeTimeDomainWaveformCheckbox = document.getElementById(
  "include-audio-waveform"
);
const collectButtonsDiv = document.getElementById("collect-words");
const startTransferLearnButton = document.getElementById(
  "start-transfer-learn"
);
...
(async function () {
  ...
  recognizer
    ...
    .catch((err) => {
      logToStatusDisplay("Failed to load model for recognizer: " + err.message);
    });
})();
  ...
  for (const word of transferWords) {
    ...
    button.addEventListener("click", async () => {
      ...
      if (word === BACKGROUND_NOISE_TAG) {
        ...
        collectExampleOptions.durationSec = Number.parseFloat(
          durationInput.value
        );
...
enterLearnWordsButton.addEventListener("click", () => {
  ...
  enterLearnWordsButton.disabled = true;
  transferWords = learnWordsInput.value
    .trim()
    .split(",")
    .map((w) => w.trim());
  transferWords.sort();
...
startTransferLearnButton.addEventListener("click", async () => {
  ...
  function plotLossAndAccuracy(epoch, loss, acc, val_loss, val_acc, phase) {
    ...
    Plotly.newPlot(
      "loss-plot",
      [
        trainLossValues[INITIAL_PHASE],
        valLossValues[INITIAL_PHASE],
        trainLossValues[FINE_TUNING_PHASE],
        valLossValues[FINE_TUNING_PHASE],
      ],
      {
        width: 480,
        height: 360,
    ...
    Plotly.newPlot(
      "accuracy-plot",
      [
        trainAccValues[INITIAL_PHASE],
        valAccValues[INITIAL_PHASE],
        trainAccValues[FINE_TUNING_PHASE],
        valAccValues[FINE_TUNING_PHASE],
      ],
      {
        width: 480,
        height: 360,
  ...
  disableAllCollectWordButtons();
  const augmentByMixingNoiseRatio = document.getElementById(
  ...
  datasetFileReader.onload = async (event) => {
    try {
      ...
      const evalResult = await transferRecognizer.evaluate({
        windowHopRatio: 0.25,
        wordProbThresholds: [
          0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.5, 0.55, 0.6, 0.65,
...
datasetIOButton.addEventListener("click", () => {
  if (datasetIOButton.textContent.endsWith(" >>")) {
    ...
    datasetIOButton.textContent = datasetIOButton.textContent.replace(
      " >>",
      " <<"
    );
  } else {
